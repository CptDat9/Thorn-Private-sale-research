...
import "./interfaces/IThornERC20.sol";
...
contract OlympusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SThorn }

    address public immutable Thorn;
    ...
    address public sThorn;
    uint public sThornQueue; // Delays change to sThorn address
    ...
    constructor (
       ...
       // isLiquidityToken[ _ThornDAI ] = true;
       // liquidityTokens.push( _ThornDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for Thorn
        @param _amount uint
        @param _token address
        @param payout uint
        @return send_ uint
     */
    function deposit( uint _amount, address _token, uint payout) external returns ( uint send_ ) {
        ...
        // mint Thorn needed and store amount of rewards for distribution
        // send_ = value.sub( _profit );
        IERC20(Thorn).transfer(msg.sender, payout);
    ...
    // /**
    //     @notice allow approved address to burn Thorn for reserves
    //     @param _amount uint
    //     @param _token address
    //  */
    ...
    //     uint value = valueOf( _token, _amount );
    //     IThornERC20( Thorn ).burnFrom( msg.sender, value );
    ...
    //     uint maximumDebt = IERC20( sThorn ).balanceOf( msg.sender ); // Can only borrow against sThorn held
    ...
    // /**
    //     @notice allow approved address to repay borrowed reserves with Thorn
    //     @param _amount uint
    //  */
    // function repayDebtWithThorn( uint _amount ) external {
    //     require( isDebtor[ msg.sender ], "Not approved" );

    //     IThornERC20( Thorn ).burnFrom( msg.sender, _amount );

    //     debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
    //     totalDebt = totalDebt.sub( _amount );

    //     emit RepayDebt( msg.sender, Thorn, _amount, _amount );
    // }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20(Thorn).transfer(_recipient, _amount);
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Thorn ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Thorn valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Thorn decimals
            value_ = _amount.mul( 10 ** IERC20( Thorn ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SThorn ) { // 9
            sThornQueue = block.number.add( blocksNeededForQueue );
        } else return false;
