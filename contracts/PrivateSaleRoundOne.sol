...
contract PrivateSaleRoundOne is Policy {
    ...
    event UpdateWhiteListSucessfully(address user, bool status);
    /* ======== STATE VARIABLES ======== */
    ...
    function toggleWhitelisted(
        ...
        require(isUseWhiteList, "WhiteList is not used");
        bool _isWhiteList = isWhiteList[_address];
        isWhiteList[_address] = !_isWhiteList;
        emit UpdateWhiteListSucessfully(_address, isWhiteList[_address]);
    }

    function toggleUseWhiteList() external onlyPolicy() {
        bool _isUseWhiteList = isUseWhiteList;
    ...
    function deposit(
        ...
        require(
            ...
            "It's not time to buy token in this terms yet!"
        );
        require(
            block.timestamp <= terms.buyingTimeStart + terms.buyingTime,
            "terms private sale is expired!"
        );
        ...
        require(
            totalDebt <= terms.maxDebt,
            "Max Capacity has reached"
        );

        require(amountTHORN + bondInfo[_depositor].totalBought  <= getPayout(terms.maxPayout), "THORN too max");
    ...
    function getTotalReceived(address _depositor) public view returns(uint256){
        PriSale memory private_sale = bondInfo[_depositor];
    ...
    function bondPrice() public view returns (uint256){
        return thornPrice.mul(uint(100000).sub(terms.discountRatio)).div(100000);
    }
    ...
    function getPayout(uint256 _amount) public view returns (uint256) {
        ...
        uint payout = _amount.mul(payoutRate)
                        // .mul( 10 ** 18 )
    ...
    function getWithdrawableAmount(
        ...
        if (percentVested >= 10000) {
            ...
            return (info.payout.mul(percentVested).div(10000)).add(info.amountClaim);
        }
    }
    ...
    function getBondInfo(address _depositer) public view returns(PriSale memory) {
        return bondInfo[_depositer];
    }

    function getMaxPayout(address _depositor) public view returns(uint256) {
        uint256 maxPayout = getPayout(terms.maxPayout).sub(bondInfo[_depositor].totalBought);
